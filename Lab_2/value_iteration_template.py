...
def get_action_value(mdp, state_values, state, action, gamma):
    ...
    return np.sum([prob * (mdp.get_reward(state, action, next_state) + gamma * state_values[next_state]) for next_state, prob in next_states.items()])
...
def get_new_state_value(mdp, state_values, state, gamma):
    ...
    if mdp.is_terminal(state):
        return 0
    new_state_values = [get_action_value(mdp, state_values, state, a, gamma) for a in mdp.get_possible_actions(state)]
    return max(new_state_values)
...
def get_optimal_action(mdp, state_values, state, gamma=0.9):
    ...
    q = {a: get_action_value(mdp, state_values, state, a, gamma) for a in actions}
