...
def select_elites(states_batch, actions_batch, rewards_batch, percentile=30):
    """
...
def update_policy(elite_states, elite_actions, n_states, n_actions):
    ...
    for i, sum_i in enumerate(np.sum(new_policy, axis=1)):
        if sum_i == 0:
            new_policy[i] = np.ones(n_actions) / n_actions
        else:
            new_policy[i] = new_policy[i] / sum_i
...
def rl_cross_entropy():
    ...
    for i in range(total_iterations):
        ...
        new_policy = update_policy(elite_states, elite_actions, 500, 6)
